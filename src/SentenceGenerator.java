...
public class SentenceGenerator {
	...
	final static String eos_not_found = "<EOS_NOT_FOUND>";
	String lastSentence;
	...
	public Word getRandomWord() {
		...
		return getWordFromList((int) (Math.random() * wordListSize));
	}
	...
	private Word getWordFromList(int index) {
		return this.repository.getWordList().get(index);
	}
...
//        System.out.println("Starting word: " + startingWord);
	...
	public String buildString(Word start, int numOfWords, boolean firstTime) {
		...
		Word repoWord = repository.getByName(start.toString());
		if (repoWord != null) {
			next = repoWord;
		}
	...
	private String getEOSWord(Word next) {
		...
		String returnVal = eos_not_found;
		Word likelyEOSWord = repository.getLikelyEOSWord(next);

		while (likelyEOSWord == null) {
			next = getRandomWord();
			likelyEOSWord = repository.getLikelyEOSWord(next);
		}

		exceptions.add(likelyEOSWord);
		returnVal = likelyEOSWord.toString();
